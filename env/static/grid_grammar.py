...
if __name__=='__main__':
    ...
    for i in range(1, 11):
        ...
        # Generate 60 unique trials (20 per rule)
        unique_trials = []
        for rule in ['chain', 'tree', 'loop']:
            for _ in range(20):
                ...
                grid = generate_grid(rules=rule, trial_index=trial_index, image_pairs=current_pair, repeat_index=None, n=n)
                trial_index += 1
                unique_trials.append(grid)
        
        # Shuffle the unique trials
        random.shuffle(unique_trials)
        
        # Create the main trials list with repeats
        main = []
        repeated_trials = set()
        
        # First, add all unique trials
        for trial in unique_trials:
            main.append(trial)
        
        # Then, add repeats ensuring they are at least 8 trials apart
        for i, trial in enumerate(unique_trials):
            # Find a position between 8 to 12 trials after the original
            min_position = i + 5
            max_position = i + 15
            
            if min_position >= len(main):
                # If we can't place it 8 trials after, place it at the end
                position = len(main)
            else:
                # Find a random position between 8 to 12 trials after
                position = random.randint(min_position, max_position)
            
            # Create a copy of the trial with updated trial number and repeat_index
            repeated_trial = trial.copy()
            repeated_trial['trial_number'] = trial_index
            repeated_trial['repeat_index'] = i  # Index of the original trial
            trial_index += 1
            
            # Insert the repeated trial at the chosen position
            main.insert(position, repeated_trial)

        # Verify we have exactly 120 trials
        if len(main) != 120:
            raise ValueError(f"Expected 120 trials but got {len(main)} trials")
